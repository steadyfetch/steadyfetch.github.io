...
typography:
  ...
  label:
    ...
    fontVariation: "'wdth' 88"
  nav:
    fontFamily: "Archivo, 'Archivo Nav Fallback', 'Helvetica Neue', Helvetica, Arial, sans-serif"
    fontSize: "0.715rem"
    fontWeight: 600
...
- Zero layout shift: self-hosted, preloaded variable woff2 with metric-matched fallbacks for both faces
...
**Body Font:** Archivo, the same self-hosted file (with `Archivo Fallback` at 98.7%, metric-matched on Arial / Liberation Sans). Nav links take `--ui` (the stylesheet's only `--ui` consumer; labels, crumbs and action links stay on `--sans`), backed by `Archivo Nav Fallback` at 86.3% — they are set at `font-stretch: 86%`, which no fallback can condense, and they were what visibly wrapped without it.
...
- **Label** (600, `wdth` 88, .705rem, .1em, uppercase, `--ink-2`): unit labels under figures, plate counts, "ways" headings. Nav links (.715rem, `wdth` 86, .06em, in `--ui`), crumbs (.75rem, .085em), action links (.715rem, `wdth` 90, .075em) and table headers (.655rem, .09em) are the same voice at neighbouring sizes.
...
- **Nav links:** label voice in `--ui` (.715rem, 600, `wdth` 86, .06em, uppercase, `--ink-2`) with a transparent 1px bottom border that becomes `--mark` on hover and on `aria-current="page"`, when the text also goes `--ink`. Nav pushes right with `margin-left: auto`.
